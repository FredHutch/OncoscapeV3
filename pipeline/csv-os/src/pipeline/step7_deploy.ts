...
export class Deploy {
  public static All(dir: string): Promise<any> {
    return new Promise((resolve, reject) => {
      IO.DeleteArtifacts().then(() => {
        const projectId = dir + '_' + uuidv1().replace(/\s/gi, '');
        // const projectId = 'acsp-e484b460-a0bf-11e8-9654-2973b6f27a97';
          this.CreateS3Bucket(projectId).then(() => {
            Promise.all([
              this.CreateDynamoRecord(
                'michael@zager.co',
                projectId,
                'CBIO',
                'site',
        ...
        (err: any, data: any) => {
          if (data !== null) {
            const folderNames = data.CommonPrefixes.map(
              (v: any) => v.Prefix.split('/')[1]
            );
            if (folderNames.indexOf(projectId)) {
              console.log(
                'GUID ALREADY EXISTS.. This is pretty much impossible other than during testing'
              );
            }
          }
  ...
  public static CreateDynamoRecord(
    email: string,
    projectId: string,
    name: string,
    site: string,
