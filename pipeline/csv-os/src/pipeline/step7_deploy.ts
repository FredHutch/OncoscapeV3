...
export class Deploy {
  ...
  public static CreateS3Bucket(projectId: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      s3.listObjectsV2({ Bucket: bucket, Delimiter: '/', Prefix: 'datasets/', EncodingType: 'url' }, (err: any, data: any) => {
        ...
        Promise.all(
          files.map((file: string) => {
            return new Promise((resolve, reject) => {
              ...
              var params = { Bucket: bucket, Key: 'datasets/' + projectId + '/' + file, ContentType: 'application/json', ContentEncoding: 'gzip', ACL: 'private', Body: rstream }; // Remove ACL For Production
