...
const fs = require('fs');
const files = fs.readdirSync(process.cwd()+'/data').map(v => v.toLowerCase()).filter(v => v.indexOf('patientevent') === 0);
const eventMap = files.map(v => {  return v.split('-').filter(w => w !== 'patientevent').map(v => v.replace('.csv','')) }).reduce( (p, c) => { 
        p[c[1]] = c[0]; return p; }, {});
...
fs.writeFileSync(process.cwd()+'/output/manifest-events-patient.json', JSON.stringify(eventMap), {'encoding':'UTF-8'});
fs.writeFileSync(process.cwd()+'/output/events-patient.json', JSON.stringify(output), {'encoding':'UTF-8'});
