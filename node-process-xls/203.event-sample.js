...
const fs = require('fs');
const files = fs.readdirSync(process.cwd()+'/data').map(v => v.toLowerCase()).filter(v => v.indexOf('sampleevent') === 0);
const eventMap = files.map(v => {  return v.split('-').filter(w => w !== 'sampleevent').map(v => v.replace('.csv','')) }).reduce( (p, c) => { 
        p[c[1]] = c[0]; return p; }, {});
...
files.forEach( (file, fileIndex) => { 
    ...
    const pidIndex = cols.indexOf('SAMPLEID');
...
fs.writeFileSync(process.cwd()+'/output/manifest-events-sample.json', JSON.stringify(eventMap), {'encoding':'UTF-8'});
fs.writeFileSync(process.cwd()+'/output/events-sample.json', JSON.stringify(output), {'encoding':'UTF-8'});
