...
import { VisualizationNameFromValue } from './app/model/enum.model';
...
import { savedPointsCompute } from './app/component/visualization/savedpoints/savedpoints.compute';
...
import { TableLoaderCompute as tableLoaderCompute } from './app/component/visualization/tableLoader/tableLoader.compute';
...
import { SVRCompute } from './app/component/visualization/svr/svr.compute';


onmessage = e => {
  const me = self as DedicatedWorkerGlobalScope;
  if (!me.hasOwnProperty('util')) {
    const visId:number = parseInt(e.data.visualization);
    const visName = VisualizationNameFromValue(visId);
    console.log(`Init Compute Thread for vis: [${JSON.stringify(visId)}, ${visName}].`);
  ...
  try {
    switch (e.data.visualization) {
      ...
      case VisualizationEnum.SAVED_POINTS:
        savedPointsCompute(e.data, me);
        break;
      case VisualizationEnum.TABLE_LOADER:
        tableLoaderCompute(e.data, me);
        break;
    ...
    console.log('ERROR in compute.ts. ' + JSON.stringify(err) +'!')    ;
  }
};
