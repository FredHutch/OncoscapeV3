...
export class ComputeEffect {
  ...
  selectSamples: Observable<any> = this.actions$.pipe(ofType(compute.COMPUTE_SELECT_SAMPLES)).pipe(
    ...
    switchMap((value: [any, State], index: number) => {
      ...
      const database = value[1].graphA.config.database;
      return observableFrom(this.dataService.getPatientIdsWithSampleIds(database, sampleIds)).pipe(
        switchMap(patientIds => {
          return observableFrom(this.dataService.getPatientStatsText(database, patientIds, sampleIds)).pipe(
            mergeMap(data => {
              return [
  ...
  saveSampleSelection: Observable<any> = this.actions$.pipe(ofType(compute.COMPUTE_SELECT_SAMPLES_SAVE)).pipe(
    ...
    switchMap((value: [any, State], index: number) => {
      ...
      return observableFrom(this.dataService.getPatientIdsWithSampleIds(payload.database, payload.cohort.sids)).pipe(
        switchMap(data => {
  ...
  loadMiniBatchDictionaryLearning: Observable<any> = this.actions$
    ...
    .pipe(
      map((action: UnsafeAction) => action.payload),
      switchMap(payload => {
        return this.computeService.miniBatchDictionaryLearning(payload['config']).pipe(
          mergeMap(result => {
            return [
              result === null
                ? new NullDataAction()
  ...
  loadLinearDiscriminantAnalysis: Observable<any> = this.actions$
    ...
    .pipe(
      map((action: UnsafeAction) => action.payload),
      switchMap(payload => {
        return this.computeService.linearDiscriminantAnalysis(payload['config']).pipe(
          mergeMap(result => {
            return [
              result === null
                ? new NullDataAction()
