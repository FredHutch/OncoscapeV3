...
export class HazardGraph extends AbstractVisualization {
  public set data(data: HazardDataModel) {
    this._data = data;
  }
  public get data(): HazardDataModel {
    return this._data as HazardDataModel;
  }
  public set config(config: HazardConfigModel) {
    this._config = config;
  }
  public get config(): HazardConfigModel {
    return this._config as HazardConfigModel;
  }
  ...
  addObjects(type: EntityTypeEnum): void {
    ...
    const hX = scaleLinear()
      .range([-500, 500])
      .domain(
        this.data.result.hazard.reduce(
          (p, c) => {
    ...
    ChartFactory.configPerspectiveOrbit(
      this.view,
      new THREE.Box3(new THREE.Vector3(-500, -500, -5), new THREE.Vector3(500, 500, 5))
    );
